...
async def http2(reader, writer):
    ...
    while(True):
        ...
        if(frame_type is "HEADERS"):
            content, content_type, status, additional = b"", "", 404, ""
            request = {}
            ...
            repb = {}
            for a in request:
                repb[a.encode("utf-8")] = request[a].encode("utf-8")
            if(request[":method"] == "GET"):
                if(request[":path"].find("?") != -1):
                    print("GEPT...>>> ", request[":path"])
                    pfg = request[":path"].split("?")
                    content, content_type, status, additional = gept.gept(pfg[0], pfg[1].encode("utf-8"), message_dir + "/412.html", repb)
                else:
                    ...
                    while True:
                        ...
                        if(Type == b"\x00"):
                            ...
                            if(flags & 0x01):
                                break
                    print(len(post_data))
                content, content_type, status, additional = post.post(request[":path"], post_data, message_dir, repb)
            Adit = []
            if(additional != ""):
                additional = additional.split("\r\n")
                for a in additional:
                    i = a.find(": ")
                    Adit.append((a[:i].lower(), a[i+2:]))
