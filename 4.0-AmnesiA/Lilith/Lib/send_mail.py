...
import socket
import ssl
import Lilith.Lib.parser as parser
...
def get_mail_servers(domain):
    ...
    for a in range(QDCOUNT):
        sections = parser.bytes_parse(
            sections[-1], (sections[-1].find(b"\x00")+1, 2, 2))
    for a in range(ANCOUNT):
        tmp = sections[:-1]
        if(sections[-1][0] & 0b11000000):
            tmp += parser.bytes_parse(sections[-1], (2, 2, 2, 4, 2))
        else:
            tmp += parser.bytes_parse(sections[-1],
                                      (sections[-1].find(b"\x00")+1, 2, 2, 4, 2))
        ...
        while(i < len(mail_server)):
            ...
            if((leng >> 6) == 0b11):
                ...
                if(st != b"\x00"):
                    name += st + b"."
                i += leng+1
        servers.append([int.from_bytes(preference, "big"),
                        name[:-2].decode("utf-8")])
    return servers
